...
class CryptoState():
    print('Introduce la Crypto de inicio, debe estar en esta lista')
    print(supported_coin_list)
    cryptoinit = str(input()).upper()
    print('Start in ', cryptoinit)	
    ...
    def __init__(self):
        if(os.path.isfile(self._coin_backup_file) and os.path.isfile(self._table_backup_file)):
            ...
            self.current_coin = self.cryptoinit
            self.coin_table = coin_table
        else:
            current_coin = self.cryptoinit
            if (not current_coin in supported_coin_list):
                exit(
                    "***\nERROR!\nHas introducido una moneda no valida\n***")
        ...
        def f(*args, **kwargs):
            ...
            while attempts < howmany:
                try:
                    ...
                    print("Intento fallido de Compra/Venta. Intentandolo de nuevo")
                    if attempts == 0:
                        logger.info(e)
                        attempts += 1
        return f
...
def sell_alt(client, alt_symbol, crypto_symbol):
    ...
    logger.info('Vendiendo %s de %s' % (order_quantity, alt_symbol))
    print('Vendiendo ', order_quantity, 'de ', alt_symbol)

    bal = get_currency_balance(client, alt_symbol)
    logger.info('El precio es %s' % bal)
    print('Cantidad vendida: ', bal)
    order = None
    while order is None:
    ...
    logger.info(stat)
    print('stat', stat[u'symbol'], ' ', stat[u'executedQty'], ' ', stat[u'status'])
    while stat[u'status'] != 'FILLED':
        logger.info(stat)
        print('stat Filled')
        stat = client.get_order(
...
def initialize_trade_thresholds(client):
    ...
    print("Arranque finalizado, generando archivo")
    with open(g_state._table_backup_file, "w") as backup_file:
        json.dump(g_state.coin_table, backup_file)
...
def scout(client, transaction_fee=0.001, multiplier=5):
    ...
    '''
    logger.info("Explorando...")
    print(datetime.datetime.now(), 'Explorando...')
    global g_state
    ...
    for optional_coin in [coin for coin in g_state.coin_table[g_state.current_coin].copy() if coin != g_state.current_coin]:
        ...
        if (coin_opt_coin_ratio - transaction_fee * multiplier * coin_opt_coin_ratio) > g_state.coin_table[g_state.current_coin][optional_coin]:
            ...
            print('Saltamos de ', g_state.current_coin, ' a ', optional_coin)
            break
